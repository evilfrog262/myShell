...
void parseCommand(char *cmd) {
  ...
  while (token != NULL && tokenCount < 256) {
    ...
    // if it is a python script and the first command, take note
    if (tokenCount == 0 && token[strlen(token) - 1] == 'y' && token[strlen(token) - 2] == 'p'
	&& token[strlen(token) - 3] == '.') {
      // make python interpreter the first argument
      allTokens[0] = "/usr/bin/python";
      allTokens[1] = token; // name of python script
      tokenCount += 2;
      token = strtok(NULL, " ");
      continue;
    }
	...
	outFile[strlen(outFile) - 1] = 0;
      ...
      token = strtok(NULL, " ");
      // should not be any more args unless it's an &!
    ...
    // & has already been encountered
    if (isBackground) {
      // should not have any chars after the &
      printError();
      return;
    }
